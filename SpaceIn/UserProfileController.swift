...
extension UserProfileController {
    ...
    private func handleBlockFireDB() {
        
        guard let fromId = Auth.auth().currentUser?.uid else { return }
        guard let toId = self.userForProfile?.userId else { return }
        
        let blockRef = Database.database().reference().child("users").child(fromId).child("block")
        let blockValue = [toId: 1]
        
        blockRef.updateChildValues(blockValue) { (error, blockRef) in
            if error != nil {
                print(error!)
            ...
            let blockedRef = Database.database().reference().child("users").child(toId).child("blocked")
            let blockedValue = [fromId: 1]
            
            blockedRef.updateChildValues(blockedValue) { (error, blockRef) in
                if error != nil {
                    print(error!)
...
extension UserProfileController: SKPSMTPMessageDelegate {
    ...
    fileprivate func handleReportEmail() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5, execute: {
            ...
            let messageBody = "user \(fromId) reported user \(toId)"
